...
func (r *DeviceRepo) ListAll(ctx context.Context) ([]domain.Device, error) {
	rows, err := r.db.QueryContext(ctx, `SELECT id, device_id, status, last_seen FROM devices ORDER BY last_seen DESC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *DeviceRepo) UpdateStatus(ctx context.Context, deviceID string, status domain.DeviceStatus) error {
	res, err := r.db.ExecContext(ctx, `UPDATE devices SET status = $1 WHERE device_id = $2`, status, deviceID)
	if err != nil {
		return err
	}
...
func (r *DeviceRepo) UpdateLastSeen(ctx context.Context, deviceID string) error {
	_, err := r.db.ExecContext(ctx,
		`UPDATE devices SET last_seen = $1 WHERE device_id = $2`, time.Now(), deviceID,
	)
	return err
}
